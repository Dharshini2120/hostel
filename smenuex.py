...
for j in re:
    na=j[2]
s = """select * from req_menu where student_name='%s'  """%(na)
b.execute(s)
record = b.fetchall()
        ...
        print("""
                        <tbody>
                            <tr>
                               ...
                               <input type="submit" class="btn btn-danger" value="Delete" name="sub" >
                               </form></td>
                               </tr>
                        </tbody>
                    """ % (h, i[1], i[4], i[5],i[0]))
...
if sub != None:
    qq = """Update req_menu set Status='Deleted' where id='%s' """ % (De)
    b.execute(qq)
    a.commit()
    print("""<script>alert('Your Requested Menu Deleted Successfully');</script>""")
D = reg.getvalue("d")
sub = reg.getvalue("sub")
if sub != None:
    q = """Update req_menu set Status='Deleted' where id='%s' """ % (D)
    b.execute(q)
    a.commit()
    print("""<script>alert('Your Requested Menu Deleted Successfully');</script>""")
