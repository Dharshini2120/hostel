...
pp = """select * from vacation where Status='Requested' """
b.execute(pp)
rec = b.fetchall()
student_query = """ 
    SELECT s.Name,s.Register_Date
    FROM student s
    INNER JOIN vacation v ON v.Student_Name = s.Name """
...
print("""
 <div class="col-lg-9" style="margin-left:20px; margin-top:40px;"> 
    <h1 style="font-family:Times New Roman; text-align:center; color:black; text-shadow: 2px 3px 5px  black; font-weight:500;">Vaction Details</h1>
...
<td>%s</td>
                                    <td>%s</td>
                                   <td><form method="post" style="display:flex;">
                               <input type="hidden" value="%s" name="de">
                               <input type="hidden" value="%s" name="ddd">
                               <input type="hidden" value="%s" name="sn">
                               <input type="submit" class="btn btn-success" value="Approve" name="sub" >
                               <input type="hidden" value="%s" name="d">
                                <input type="submit" class="btn btn-danger" value="Reject" name="su" style="margin-left:20px;" >
                               </form></td>
                                   </tr>

                        """ % (h, i[1],jd, i[2], i[4],i[8],i[9], i[0], i[4],i[2],i[0]))
        h += 1


print("</tbody></table>")
De = form.getvalue("de")
ddd=form.getvalue("ddd")
sub = form.getvalue("sub")
sn=form.getvalue("sn")


if sub != None:
    qq = """Update vacation set Status='Approved' where id='%s' """ % (De)
    b.execute(qq)
    a.commit()
    get_occupancy = """SELECT Occupancy FROM rooms WHERE Room_Number='%s'""" % (ddd)
    b.execute(get_occupancy)
    current_occupancy = b.fetchone()[0]
    new_occupancy = current_occupancy - 1
    update_occupancy = """UPDATE rooms SET Occupancy='%s' WHERE Room_Number='%s'""" % (new_occupancy, ddd)
    b.execute(update_occupancy)
    a.commit()
    update_book_status = "UPDATE book SET Status='Vacated' WHERE room_no='%s' and Student_Name='%s' "%(ddd,sn)
    b.execute(update_book_status)
    a.commit()
    print("""<script>alert('Requested Vacation Approved Successfully');</script>""")
D = form.getvalue("d")
su = form.getvalue("su")
if su != None:
    q = """Update vacation set Status='Rejected' where id='%s' """ % (D)
    b.execute(q)
    a.commit()
    print("""<script>alert('Requested Vacation Rejected Successfully');</script>""")
