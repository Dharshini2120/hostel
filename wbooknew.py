...
b.execute(v)
re = b.fetchall()
s = """select * from book where Status='Booked'"""
b.execute(s)
rec = b.fetchall()
print("""<!DOCTYPE html>
<html lang="en">
...
print("""
 <div class="col-lg-9" style="margin-left:20px; margin-top:40px;"> 
    <h1 style="font-family:Times New Roman; text-align:center; color:black; text-shadow: 2px 3px 5px  black; font-weight:500;">Booked Rooms</h1>
...
for i in rec:
    print("""
                    <tbody>
                        <tr>
                            ...
                            <td style="display:flex;">
                                ...
                                <button class="btn btn-success"  data-toggle="modal" data-target="#view%s"> Accept </button>
                               <form> 
                               <input name="idd" value='%s'type="hidden">
                ...
                """ % (h, i[1], i[3], i[5], i[6],i[7], i[0],id,i[0],i[0]))
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-lg-6 col-6">
                        ...
                        <p style="margin-top:30px;"><strong>Rent</strong></p>
                        <p style="margin-top:30px;"><strong>Due Date</strong></p>
                    </div>
                    <div class="col-lg-6 col-6">
                        <p>%s</p>
                        <p>%s</p>
                         <p>%s</p>
                         <form method="post">
                         <p><input type="text" name="mon" value="Per Semester" class="form-control" readonly></p>
                         <p><input type="text" name="ren"  class="form-control"></p>
                         <p><input type="date" name="dat"  class="form-control"></p>
                         <input type="hidden" name="d" value='%s'>
                    </div>
                </div>
            </div>
            <div class="modal-footer">
            <input type="submit" class="btn btn-info" name="sub" value="Done">
...
mon=reg.getvalue("mon")
ren=reg.getvalue("ren")
dat=reg.getvalue("dat")

d=reg.getvalue("d")
if sub!=None:
    v="""update book set Month='%s',Rent='%s',Due_Date='%s',Status='Processing' where id='%s' """%(mon,ren,dat,d)
    b.execute(v)
    a.commit()
    print("""<script>alert('Accepted Successfully');
    location.href="wbooknew.py?id=%s"</script>"""%(id))

rej=reg.getvalue("rej")
idd=reg.getvalue("idd")
dd=reg.getvalue("dd")
if rej!=None:
    s="""Update book set Status='Rejected' where id='%s'"""%(dd)
    b.execute(s)
    a.commit()
    print("""<script>alert('Rejected Successfully');
    location.href="wbooknew.py?id=%s"</script>"""%(idd))