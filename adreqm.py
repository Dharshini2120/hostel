...
s = """select * from req_menu where Status='Requested' """
b.execute(s)
record = b.fetchall()
print("""<!DOCTYPE html>
<html lang="en">
...
print("""
 <div class="col-lg-9" style="margin-left:20px; margin-top:40px;"> 
    <h1 style="font-family:Times New Roman; text-align:center; color:black; text-shadow: 2px 3px 5px  black; font-weight:500;">Requested Menu</h1>
                        ...
                        <tbody>
                            <tr>
                               ...
                               <input type="hidden" value="%s" name="d">
                                <input type="submit" class="btn btn-danger" value="Reject" name="su" >
                               </form></td>
                               </tr>
                        </tbody>
                    """ % (h, i[1],i[2],i[3], i[4], i[5], i[0],i[0]))
        h += 1
...
if sub != None:
    qq = """Update req_menu set Status='Approved' where id='%s' """ % (De)
    b.execute(qq)
    a.commit()
    print("""<script>alert('Requested Menu Approved Successfully');</script>""")
D = reg.getvalue("d")
su = reg.getvalue("su")
if su != None:
    q = """Update req_menu set Status='Rejected' where id='%s' """ % (D)
    b.execute(q)
    a.commit()
    print("""<script>alert('Requested Menu Rejected Successfully');</script>""")
